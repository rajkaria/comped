...
ONE_LINER = 'curl -fsSL "https://play.modiqo.ai/install?play={0}/comped" | sh'.format(HANDLE)
...
<p>It fetches <strong>rote</strong> — the free runner from <a href="https://www.modiqo.ai">Modiqo</a> that comped is written for — only if you don't already have it, asks you to sign in to the registry, shows you exactly what the comped Play reads and writes, and waits for your <em>yes</em>. Then it runs. About a minute the first time; ten seconds after that.</p>
<div class="callout"><p>Already have rote? Then it's just <code>rote play run https://play.modiqo.ai/{handle}/comped</code>. Check you're on 0.78 or newer with <code>rote --version</code>.</p></div>
...
<li><strong>Your comp score.</strong> That number divided by what your plan costs for the same window. <strong>13×</strong> means your subscription paid for itself thirteen times over.</li>
...
<p>Paying for something the table doesn't list — a Kimi or GLM coding plan, a team seat? <code>plan=usd:29</code> prices it.</p>
...
""".format(one=esc(ONE_LINER), handle=HANDLE,
