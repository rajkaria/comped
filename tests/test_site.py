...
class Site(unittest.TestCase):
    ...
    def test_llms_txt_offers_the_doors_the_site_leads_with(self):
        # tests/test_llms.py checks the briefing against the parser it talks to. This checks the
        # one thing that lives on the site side: an agent must be told the account-free way in.
        txt = (SITE / "llms.txt").read_text(encoding="utf-8")
        self.assertIn("https://gotcomped.com/comped.sh", txt)
        self.assertIn("npx comped", txt)
