...
# A PDF places glyphs, not words: a line can arrive as "Am ount due". Keyword tests therefore also
# run against the line with its spaces removed, which costs nothing and recovers the broken ones.
TOTAL_TIGHT = re.compile(r"(grandtotal|amount(?:due|paid|charged)|total(?:due|paid|charged|amount)|"
                         r"ordertotal|youpaid|totalcharged|total)", re.I)


def total_line(line: str) -> bool:
    return bool(TOTAL_WORD.search(line)) or bool(TOTAL_TIGHT.search(re.sub(r"\s+", "", line)))
...
def receipt_evidence(text: str, confidence: str) -> list:
    ...
    """
    head = text[:20000]
    tight = re.sub(r"(?<=\w) (?=\w)", "", head)
    found = []
    for name, pattern in EVIDENCE:
        m = pattern.search(head) or pattern.search(tight)
        if m and not NEGATED.search(m.string[max(0, m.start() - 12):m.start()]):
            found.append(name)
...
def guess_vendor(sender: str, subject: str, filename: str, text: str) -> tuple:
    ...
    for line in [l.strip() for l in str(text).splitlines() if l.strip()][:4]:
        letters = sum(1 for ch in line if ch.isalpha() or ch.isspace())
        if (2 < len(line) < 48 and letters >= 0.8 * len(line) and sum(ch.isalpha() for ch in line) > 2
                and not MONEY.search(line) and not RECEIPTISH.search(line)):
