...
Rote Playoffs hackathon entry: three rote Plays (`session-ledger`, `comped`, `wrong-turns`) on one stdlib-only Python core, plus the gotcomped.com leaderboard (`api/` on Vercel, `leaderboard/post_score.py` in the comped Play, Postgres on Supabase). comped has **two front doors on one core**: `site/comped.sh` (no account, downloads `site/comped.tar.gz`, runs, deletes itself) and `site/run.sh` (the rote Play, consent screen, needs a Modiqo account). `standalone/comped.py` is the account-free entry point; both take the same fourteen parameters, enforced by `tests/test_standalone.py`. Submissions close 7 Sep 2026 20:00 London. Session state lives in `docs/context/`, not here.
...
- Generated, never hand-edited: `plays/*/main.ts` + `deps.toml` (`tools/build_plays.py`), `plays/*/resources/` incl. comped's `post_score.py` (`tools/sync_plays.py`), `site/docs.html` + `site/developers.html` (`tools/build_site.py`), `site/comped.tar.gz` + `.sha256` (`tools/build_dist.py`, called by `build_site.py`), `resources/prices.json` (`tools/build_prices.py`).
