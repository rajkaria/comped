...
|---|---|---|
| `record` | root | `python3 resources/micro_core/cli.py streak record --did $did --state-dir $state_dir --now $now --demo $demo` |
| `report` | `record` | `python3 resources/micro_core/cli.py streak report --state-dir $state_dir --window $window --did $did --tz $tz --now $now --demo $demo` |
