...
|---|---|---|
| `record` | root | `python3 resources/micro_core/cli.py punch record --note $note --tag $tag --state-dir $state_dir --now $now --demo $demo` |
| `report` | `record` | `python3 resources/micro_core/cli.py punch report --state-dir $state_dir --days-back $days_back --tz $tz --now $now --demo $demo` |
