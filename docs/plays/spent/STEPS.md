...
|---|---|---|
| `record` | root | `python3 resources/micro_core/cli.py spent record --entry $entry --currency $currency --state-dir $state_dir --now $now --demo $demo` |
| `report` | `record` | `python3 resources/micro_core/cli.py spent report --state-dir $state_dir --budget $budget --currency $currency --tz $tz --now $now --demo $demo` |
