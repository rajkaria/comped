...
- **0.1.5 published, and a third door (05 Sep, this session).** All three Plays re-pushed to the registry at **0.1.5** (`rote registry play push plays/<slug> rajkaria`; the slug argument is the *namespace*, not the play name). Gates still 1.00 on all three; smoke run off the public 0.1.5 archive from a fresh dir passes. `npx comped` now exists: `npm/`, assembled by `tools/build_npm.py` from the same `build_dist.members()` payload, no node dependencies and no install script, `bin/comped.js` only finds an interpreter and hands it `payload/comped.py`. Verified by `npm pack` + install + run (210 KB). **The npm package is NOT published**: publishing needs Raj's own `npm publish npm/`. Also new: `site/llms.txt` (generated from the Play manifest and the bundled resources), `site/robots.txt`, `site/sitemap.xml`. **The site changes are not deployed**: Vercel builds from `main`, and this work is on a branch.
