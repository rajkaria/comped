...
5. **Voice pass on docs.html / developers.html** (`tools/build_site.py` `docs_page()` / `developers_page()`): same plain sentences as the landing page; the dashes are gone but the register is still terse-technical. Also the Play DESCRIPTION.md texts.
6. **Optional polish:** an OG image per handle (`/api/card/<handle>.svg`) so a shared rank link unfurls with the score; a "this week" window; per-provider boards as real routes. Decide `repeat_threshold` default (spec 3, real logs need 2).
