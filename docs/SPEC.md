# Comped: design and methodology

*The record model, the pricing arithmetic, the deduplication rules, the output contracts and the
testing standard behind the Plays in this repository. Written before the code and kept in step
with it: `tests/test_play_docs.py` reads this file and fails when the Plays and the spec disagree.*

*If you are here to decide whether to run something that reads your session logs, section 9 is the
part to read, and every claim in it is checked by a test named in section 11.*
...
### 3.4 Windows, plan cost, multiplier
...
Exclusions, each measured against real logs before being added: text starting with `<` (injected context), text starting with "You are" (observer/system prompts), text containing `[Request interrupted`, text under 3 content tokens or over 400 tokens (pasted documents), project dirs matching `*observer*` or `*claude-mem*`.
...
### 4.1 Terminal card (stdout of the final step)
...
### 4.7 Delta since last run

`comped-baseline.json` stores the previous run's totals, multiplier, per-model usd and repeat clusters (labels + counts). The card shows the change; the first run says "baseline saved".

## 5. Privacy and trust statements (verbatim in every description and report)
...
**Windows.** The paste needs a Unix shell, so WSL remains the answer for the one-liner. The tool underneath is standard-library Python with no Unix dependency; `cli.py` now asks for UTF-8 on stdout, without which the card's box characters raise `UnicodeEncodeError` against a Windows code page whenever stdout is a pipe. A `windows-standalone` CI job unpacks the real archive on `windows-latest` and runs it with stdout redirected, then installs the npm package and runs that too. It went green on 05 Sep and now gates the build, which is what lets the site say Windows works rather than offering it with a caveat. `npx comped` is the Windows answer; the `curl` line still needs WSL.
