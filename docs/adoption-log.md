...
| 2026-09-06 | all 21 | as published | 1 each (19), 0 (session-ledger, wrong-turns) | +0 | **Baseline before any announcement.** Twenty-one Plays published, nothing ever posted to any channel, so every count is the verification run. Registry search ranks them first or second on their own queries, so discovery is not the gap. For scale: the leading Play in the same category, `sidships/token-tab`, is at 34. Built `gotcomped.com/plays.html`, the one link that carries all twenty-one with a paste-ready line each, linked from the landing page, the footer and `llms.txt`; the site was comped-only until now. Launch pack written to `docs/launch/POSTS.md`. |
